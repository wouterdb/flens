...
public class Influxdb {
    ...
    private static char toTimePrecision(TimeUnit timeunit) {
        switch (timeunit) {
        case SECONDS:
            return 's';
        case MILLISECONDS:
    ...
    protected int sendRequest(boolean throwExc, boolean printJson) throws Exception {
        int lg = json.length();
        try {
            json.append(']');
            // byte[] content = URLEncoder.encode(json.toString(),
            // "UTF-8").getBytes(UTF_8);
            final byte[] content = json.toString().getBytes(UTF_8);
